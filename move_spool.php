...
if (isset($_GET['line_number']) && isset($_GET['material']) && isset($_GET['area']) && isset($_GET['contractor'])) {
    $line_number = $_GET['line_number'];
    $material = $_GET['material'];
    $area = $_GET['area'];
    $contractor = $_GET['contractor'];

    if(move_spool($user_id, $line_number, $material, $area, $contractor) > 0){
        echo "
            <script>
                alert('data berhasil ditambahkan');
                document.location.href = 'move_spool.php';
            </script>
                    ...
                    <?php foreach($move_spool as $index => $row): ?>
                        <tr>
                            ...
                            <?php if(empty($row['status'])): ?>
                                <td>
                                <a href="add_status.php?id=<?= $row['id'] ?>&from=move_spool">
                            ...
                            <?php else: ?>
                                <td>
                                    <a href="install.php?id=<?= $row['id'] ?>&user_id=<?= $user_id ?>&line_number=<?= $row['line_number'] ?>&material=<?= $row['material'] ?>&area=<?= $row['area'] ?>&contractor=<?= $row['contractor'] ?>&created_at=<?= $row['created_at'] ?>&from=move_spool">
